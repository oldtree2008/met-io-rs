...
pub struct RadialDataEx {
    ...
    pub rs: Vec<Vec<Vec<Vec<f64>>>>,   //仰角->方位角->斜距   米为单位
...
impl RadialDataEx {
    ...
    pub fn ppi_to_grid(
        ...
        grid_value.iter_mut().enumerate().for_each(|(i, d)| {
            ...
            let elv_rs = &self.rs[element_idx][ele_idx][az_idx];
...
impl Default for RadialDataEx {
    fn default() -> Self {
        Self {
            ...
            rs: vec![vec![vec![vec![0f64]]]],   //仰角->方位角->斜距   米为单位
            data: vec![vec![vec![vec![0f32]]]], //物理量->仰角->方位角->值
        }
    }
}

impl RadarData for RadialDataEx {
