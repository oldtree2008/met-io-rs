...
impl RadarPDReader {
    pub fn new(fname: &str) -> Result<Self, MetError> {
        ...
        // let station_name = String::from_utf8_lossy(&station.station_);
        let phycount = elements.len();
        ...
        let mut eles = Vec::new(); //仰角
        let mut azs = Vec::new(); //方位角
        let mut rs = Vec::new();
        let mut data = Vec::with_capacity(phycount); //所有数据
        let mut data_map = HashMap::new();
        for i in 0..phycount {
            data_map.insert(i,Vec::new());
        }
        let mut datas_map = HashMap::new();
        for i in 0..phycount {
            let mut hb = HashMap::new();
            for j in 0..observe.layers.len() {
                hb.insert(j,Vec::new());
            }
            datas_map.insert(i,hb);
        }
        let mut datas;
        // let mut el_linedata;
        for (ll,layer) in observe.layers.iter().enumerate() {
            let mut first = true;
            ...
            let mut first_ele = true;
            // let mut el_linedata = Vec::new();
            for pidx in 0..phycount {
                datas = data_map.get_mut(&pidx).unwrap();
                for r in 0..layer.radial_count {
                    ...
                    let mut hb = datas_map.get_mut(&pidx).unwrap();
                    let mut hb:&mut Vec<_> = hb.get_mut(&ll).unwrap();
                    // el_linedata = datas_map[&pidx][&ll].clone();
                    hb.push(line_data);
                    datas.push(hb.clone());
                }
        ...
        dbg!(&data.len(),&data[0].len(),&data[0][0].len(),&data[0][0][0].len());
